    ...
    <div class="form-group">
        ...
        <?= $this->Form->select('experince_id', \App\Model\Service\Core::$experience, ['default' => $user->experince_id, 'empty' => 'Select experience', 'required' => true, 'class' => 'form-control pointer required']); ?>

    </div>
    <div class="form-group">
        <label class="form-control-label"> <?=  __('Role') ?> </label>
        <?= $this->Form->select('role', ['user'=>'User','admin'=>'Admin','trader'=>'Trader','account_rep'=>'Account Rep'], ['default' => $user->role, 'empty' => 'Select role', 'required' => true, 'class' => 'form-control pointer required']); ?>
    </div>
