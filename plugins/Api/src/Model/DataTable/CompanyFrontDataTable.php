    ...
    public function ajaxManageCompanySearch($requestData, $language = null)
    {
        ...
        foreach ($results as $row) {
            ...
            if ($stockInfo['info']['open'] > 0) {
                $change = (($price) * 100 / $stockInfo['info']['open']);
            }

            $class = "text-danger";
            $icon = 'fas fa-caret-down text-danger';
            if ($change >= 0) {
                $class = "text-success";
                $icon = "text-success fas fa-caret-up";
            }

            $change = number_format($change, 2, '.', '');

            $nestedData = [];
            $nestedData[] = '<a class="' . $class . '" href="' . $language . '/symbol/' . $row["symbol"] . '" ><span class="' . $class . '">' . $row["symbol"] . '</span></a>';
            $nestedData[] = $row["name"];
            $nestedData[] = '$' . number_format($stockInfo['info']['open'], 2, '.', ' ');
            $nestedData[] = '<i class="' . $icon . '"></i><span class="' . $class . '"> ' . $change . '%</span>';
            $nestedData[] = '$' . number_format($price, 2);
            $nestedData[] = $stockInfo['info']['volume'];
