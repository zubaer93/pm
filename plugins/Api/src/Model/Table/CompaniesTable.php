...
class CompaniesTable extends Table
{
    ...
    public function initialize(array $config)
    {
        ...
        $this->hasMany('Watchlist', ['foreignKey' => 'company_id', 'joinType' => 'INNER']);
    }

    public function afterSave($entity)
    {
        $searchSummaryObj = TableRegistry::get('Api.SearchSummary');
     ...
     * @return string
     */
    public function getCompanyInfo($symbol, $language, $user_id = null)
    {
        try{
            $query = $this->find()
            ...
            if ($user_id) {
                $query->contain(['Watchlist' => function ($q) use ($user_id) {
                    return $q->select(['id','user_id','group_id','group_name'=>'WatchlistGroup.name','company_id'])->where(['Watchlist.user_id' => $user_id])->contain(['WatchlistGroup']);
                }]);
            }
     ...
     *
     * @param string $language Symbol of a company
     * @return array
     */
     ...
     *
     * @param string $sector Symbol of a company
     * @param string $language
     * @param int $limit
     * @return array
     */
     ...
     *
     * @param string $industry Symbol of a company
     * @param string $language
     * @return array
     */
    ...
    /*
     * */
    public function getCompanyLang($symbol)
    {
        $exObj = TableRegistry::get('Api.Exchanges');
        $countryObj = TableRegistry::get('Api.Countries');
        $com = $this->find()->contain(['Exchanges'=>['Countries']])
            ->where(['Companies.enable !=' => 1])
            ->where(['Companies.symbol' => $symbol])
            ->first();
        return $com['exchange']['country']['market'];
    }
    ...
    public function getTrendingCompanies($language)
    {
        ...
        foreach ($companies as $key => $company) {
            $info = $this->getStocksInfo2([$company->symbol], $language);
     ...
     * getCompanyTickerInfo method This method will return the company info for ticker
     *
     * @param string $symbols Symbol of a company
     * @param string $language currently support USD|JMD
     * @return array
     */
    public function getStocksInfo($symbols, $language)
    {
        if ($language == self::USD) {
            foreach ($symbols as $symbol) {
                $companyInfo = is_string($symbol) ? $this->getCompanyInfo($symbol, $language) : $symbol;
                $array = $this->Stocks->getStockInformation($companyInfo['symbol']);
                $stocksInfo[] = [
                    'open' => $array['info']['open'],
                    'close' => $array['info']['close'],
                    'symbol' => is_string($symbol) ? $symbol : $symbol->symbol,
                    'volume' => $array['info']['volume'],
            ...
            foreach ($symbols as $symbol) {
                $companyInfo = is_string($symbol) ? $this->getCompanyInfo($symbol, $language) : $symbol;
                $array = $this->Stocks->getStockInformationLocal($companyInfo['symbol'], $companyInfo['id']);
                $stocksInfo[] = [
                    'open' => $array['info']['open'],
                    'close' => $array['info']['close'],
                    'symbol' => is_string($symbol) ? $symbol : $symbol->symbol,
    ...
    public function getStocksInfo2($symbols, $language)
    {
        if ($language == self::USD) {
            foreach ($symbols as $symbol) {
    ...
    public function getStocksInfo3($symbols)
    {
        foreach ($symbols as $symbol) {
            $language = $this->getCompanyLang($symbol);
            $companyInfo = $this->getCompanyInfo($symbol, $language);
            if ($language == self::USD) {
                $array = $this->Stocks->getStockInformation($companyInfo['symbol']);
            } else {
     ...
     *
     * @param string $company_id Symbol of a company
     * @param string $Language currently support USD|JMD
     * @return string
     */
    ...
    public function getSearchCompany($language, $search)
    {
        $query = $this->find()
            ->where([
                'OR' => [
            ...
            ->where(['enable !=' => 1])
            ->select(['id','company_name' => 'Companies.name','symbol'])
            ->toArray();
        return $query;
    }
}
