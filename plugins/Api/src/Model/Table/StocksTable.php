...
class StocksTable extends Table
{
     ...
     * @param type $language
     */
    public function getStockTopPerformInformation($language)
    {
        //dd("hello");
        $data = $this->find('all', [
     ...
     * @param type $language
     */
    public function getStockWorstPerformInformation($language)
    {
        $data = $this->find('all', [
            'fields' => [
        ...
        try {
            ...
            if (isset($jsonarray_min['optionChain']['result'][0]['quote'])) {
                return [
                    ...
                    'info' => [
                        'open' => (isset($jsonarray_min['optionChain']['result'][0]['quote']['regularMarketPrice'])) ? $jsonarray_min['optionChain']['result'][0]['quote']['regularMarketPrice'] : 0,
                        'high' => (isset($jsonarray_min['optionChain']['result'][0]['quote']['regularMarketDayHigh'])) ? $jsonarray_min['optionChain']['result'][0]['quote']['regularMarketDayHigh'] : 0,
                        'low' => (isset($jsonarray_min['optionChain']['result'][0]['quote']['regularMarketDayLow'])) ? $jsonarray_min['optionChain']['result'][0]['quote']['regularMarketDayLow'] : 0,
                        'close' => (isset($jsonarray_min['optionChain']['result'][0]['quote']['regularMarketPreviousClose'])) ? $jsonarray_min['optionChain']['result'][0]['quote']['regularMarketPreviousClose'] : 0,
                        'volume' => (isset($jsonarray_min['optionChain']['result'][0]['quote']['regularMarketVolume'])) ? $jsonarray_min['optionChain']['result'][0]['quote']['regularMarketVolume'] : 0,
                        'price_change' => (isset($jsonarray_min['optionChain']['result'][0]['quote']['regularMarketChange'])) ? $jsonarray_min['optionChain']['result'][0]['quote']['regularMarketChange'] : 0,
    ...
    public function getStockInformationLocal($symbol = null, $company_id)
    {
        ...
        $array = [
            ...
            'high' => $data['high'],
            'low' => $data['low'],
            'close' => $data['open'] + (-1 * $data['price_change']),
            'volume' => $data['volume'],
    ...
    public function getStockMarketDepth($symbol = null, $language, $company_id, $open_price, $eps)
    {
        ...
        if ($language == self::JMD) {
            $stockDetails = TableRegistry::get('Api.StocksDetails');
     ...
     * @return type
     */
    public function getStockMarketDepth2($symbol = null, $language, $company_id, $open_price, $eps)
    {
        $data = [];
        if ($language == self::JMD) {
            $stockDetails = TableRegistry::get('Api.StocksDetails');
            ...
            if (!is_null($query)) {
                ...
                $data['close_price'] = $query['close_price'];
                $data['volume_traded'] = $query['trade_value'];
                $data['last_trade_price'] = $query['last_traded_price'];
                $data['days_high_price'] = $query['days_high_price'];
                $data['days_low_price'] = $query['days_low_price'];
                $data['low_price_52_week'] = $query['low_price_52_week'];
                $data['high_price_52_week'] = $query['high_price_52_week'];
                $data['low_price_52_ind'] = !empty($query['low_price_52_ind']) ? $query['low_price_52_ind'] : "0"; 
                $data['high_price_52_ind'] = !empty($query['high_price_52_ind']) ? $query['high_price_52_ind'] : "0"; 
                $data['bid_price'] = $query['bid_price'];
                $data['ask_price'] = $query['ask_price'];
                $data['closeNetChange'] = $query['close_net_change'];
                $data['marketChange'] = $query['close_percent_change'];
                $data['marketValue'] = $query['totalissuedshares'];
                $data['sharesOutstanding'] = $query['market_cap'];
                $data['numOfTrades'] = $query['num_of_trades'];
                $data['preDividendAmount'] = !empty($query['pre_dividend_amount']) ? $query['pre_dividend_amount'] : "0"; 
                $data['dividend'] = !empty($query['dividend_amount']) ? $query['dividend_amount'] : "0"; 
                $data['eps'] = !empty($eps) ? $eps : "0"; 

                if($data['low_price_52_ind'] == '*'){
                    $data['low_price_52_ind'] = '0';
                }
                if($data['high_price_52_ind'] == '*'){
                    $data['high_price_52_ind'] = '0';
                }
            }
        }
        return $data;
...
//            $stock->time_zone = $data['time_zone'];
    ...
    public function getStockSimulationUSDChart($symbol, $date, $price, $quantity)
    {
        ...
        $jsonarray_min = json_decode($company_min, true);
        if (empty($jsonarray_min['Error Message'])) {
    ...
    public static function getStocksFromCompanyId($id, $language)
    {
        if (self::USD == $language) {
            self::getStStockInformation();
        }
    }

    public static function getStStockInformation($symbol = null)
    {
        try {
            $company_min = @file_get_contents('https://query1.finance.yahoo.com/v7/finance/options/' . $symbol);
            $jsonarray_min = json_decode($company_min, true);
