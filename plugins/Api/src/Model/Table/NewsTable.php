...
use Api\Model\Scrapper\Scrapper;
...
class NewsTable extends Table
{
        ...
        if (!empty($data) && isset($data->articles)) {
            foreach ($data->articles as $val) {
                ...
                if ($checkUrlInDB) {
                    ...
                    $news->body = $body;
                    $news->market = $market;
                    $news->description = (isset($val->description)) ? utf8_encode($val->description) : '';
                    $news->url = $val->url;
    ...
    /* this function will format imageurl if no image in image url link
     *
     * */
    public function formatImageUrl($news)
    {
        $news_array = [];
        foreach ($news as $n) {
            if ($result = substr($n['urlToImage'], 0, 7) == 'http://' || $result = substr($n['urlToImage'], 0, 7) == 'https:/') {
                $n['in_house'] = false;
                $news_array [] = $n;
            } else {
                $n['in_house'] = true;
                $n['urlToImage'] = Router::url($n['urlToImage']);
                $news_array [] = $n;
            }
        }
        return $news_array;
    ...
    public function formatBondNews($news)
    {
        $news_array = [];
        foreach ($news as $n) {
            if ($result = substr($n['urlToImage'], 0, 7) == 'http://' || $result = substr($n['urlToImage'], 0, 7) == 'https:/') {
