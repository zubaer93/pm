...
//        $query->andWhere($query->newExpr()->isNull($field));
    ...
    public function trashAll($conditions)
    {
        return $this->_table->updateAll(
                        [$this->getTrashField(false) => new Time()], $conditions
        );
    }
    ...
    protected function _getUnaryExpression()
    {
        return new UnaryExpression(
                'IS NOT NULL', $this->getTrashField(false), UnaryExpression::POSTFIX
        );
    }
    ...
    public function getTrashField($aliased = true)
    {
        ...
        if (empty($field)) {
            ...
            if (empty($field)) {
                $field = Configure::read('User.field');
            }
    ...
    {
        if ($association->target()->hasBehavior('Trash') && $association->isOwningSide($table) && $association->dependent() && $association->cascadeCallbacks()) {
            return true;
        }
