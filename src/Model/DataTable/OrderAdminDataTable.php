...
class OrderAdminDataTable
{

    public function ajaxOrderSearch($requestData)
    {
        $orderType = \App\Model\Service\Core::$orderType;
        $statusCore = \App\Model\Service\Core::$status;
        ...
        foreach ($results as $row) {
            ...
            $nestedData[] = '<span class="label ' . $class . '">' . $statusTable . '</span>';
            $nestedData[] = '<button class="btn btn-success btn-3d btn-sm" '
                    . 'data-toggle="modal" data-target="#confirm-delete" '
                    . 'data-name="Are you sure you want to approved?" '
                    . 'data-url="approve/' . $row["id"] . '" '
                    . 'type="submit">'
                    . 'Approve'
                    . '</button>';
            $nestedData[] = '<button class="btn btn-danger btn-3d btn-sm" '
                    . 'data-toggle="modal" data-target="#confirm-delete" '
                    . 'data-name="Are you sure you want to rejected?" '
                    . 'data-url="reject/' . $row["id"] . '" '
                    . 'type="submit">'
                    . 'Reject'
                    . '</button>';
            $data[] = $nestedData;
