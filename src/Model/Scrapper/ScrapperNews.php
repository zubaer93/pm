...
class ScrapperNews
{
            ...
            if (isset($result_href['href'])) {
                ...
                foreach ($data as $key => $value) {
                    ...
                    if ($checkIfExist) {
                        ...
                        $companies = self::getCompanyIds($description, $content, $title);
                        if ($companies) {
                            $companies = self::loadCompanies($companies);
                        }
                    ...
                    if (isset($result_href['href'])) {
                        ...
                        foreach ($data as $key => $value) {
                            ...
                            if ($checkIfExist) {
                                ...
                                $companies = self::getCompanyIds('', $content, $title);
                                if ($companies) {
                                    $companies = self::loadCompanies($companies);
                                }
    ...
    private static function getCompanyIds($description, $content, $title)
    {
        $companiesList = self::getCompanies();
        $companyIds = [];
        foreach ($companiesList as $id => $name) {
            foreach (self::$badWords as $word) {
                $name = str_replace($word, '', $name);
            }

            if (strpos(strtolower($description), strtolower($name)) !== false) {
                $companyIds[] = $id;
            }
            ...
            if (strpos(strtolower($title), strtolower($name)) !== false) {
                $companyIds[] = $id;
            }
        }
    ...
    private static function loadCompanies($ids)
    {
    ...
    private static function getCompanies()
    {
        if (empty(self::$companies)) {
            $Companies = TableRegistry::get('Companies');
            $data = $Companies->find('list')
                ->where(['Companies.name !=' => ''])
                ->group(['Companies.name']);

            self::$companies = $data->toArray();
        }
